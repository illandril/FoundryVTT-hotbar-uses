...
import { AbstractOwnedEntry } from './magic-items-2';
...
class MagicItemsModule extends SpecialtyModule<AbstractOwnedEntry> {
  constructor() {
    super(MODULE_ID, DEFAULT_MACRO_REGEX_ARRAY, (actor, itemLookupDetails) => {
    ...
    async (item) => {
      ...
      if (typeof item.item.consumption === 'number') {
        ...
        consumption = parseInt(item.item.consumption, 10);
      }
      if (consumption < 1) {
        return null;
      }
